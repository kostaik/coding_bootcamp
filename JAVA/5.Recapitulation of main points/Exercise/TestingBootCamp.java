...
public class TestingBootCamp {
	...
	public static void main(String[] args) {
		...
		for (int i=0;i<administration.sizeOfAnArrayListOfBootcamps();i++){
		System.out.println(administration.getBootCampAdministration().get(i).methodToString());
		...
		administration.addBootCampCandidate("kostas","Staikos",2);


		//Remove from the list member Marios if exist print message "success" if not "failed to find"
		if (administration.removeBootCampCandidate("Marios")){
		...
		administration.removeBootCampCandidate("Vlad");
		administration.removeBootCampCandidate("Marianna");


		//Show information for member with surname "Louis"
		System.out.println("about Louis:  "
				+ ""+administration.searchBootCampCandidate("Louis"));//??????????

		//Change age for member with name "Fisayo" and surname "Caleb" to age of 28
		administration.changeAgeOfBootCampCandidate("Fisayo", "Caleb", 28);
